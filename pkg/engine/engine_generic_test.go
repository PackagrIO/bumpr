...
func (suite *EngineGenericTestSuite) TestEngineGeneric_BumpVersion() {
	...
	//test
	berr := genericEngine.BumpVersion()
	require.NoError(suite.T(), berr)

	//assert
	require.Equal(suite.T(), "0.0.2", genericEngine.GetNextMetadata().(*metadata.GenericMetadata).Version)

}

func (suite *EngineGenericTestSuite) TestEngineGeneric_BumpVersion_Metadata() {
	//setup
...
func (suite *EngineGenericTestSuite) TestEngineGeneric_BumpVersion_Merge() {
	//setup
	...
	//assert
	require.Equal(suite.T(), "0.0.2", genericEngine.GetNextMetadata().(*metadata.GenericMetadata).Version)
	// Make sure the version file is actually merged
	expected, err := os.ReadFile(path.Join(suite.PipelineData.GitLocalPath, "VERSION_expected"))
	require.NoError(suite.T(), err)
	generated, err := os.ReadFile(path.Join(suite.PipelineData.GitLocalPath, "VERSION"))
	require.NoError(suite.T(), err)
	require.Equal(suite.T(), expected, generated)

}

func (suite *EngineGenericTestSuite) TestEngineGeneric_BumpVersion_Template() {
	//setup
