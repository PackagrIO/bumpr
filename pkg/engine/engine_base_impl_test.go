...
func TestEngineBase_BumpVersion_Patch(t *testing.T) {
	...
	fakeConfig.EXPECT().GetString("version_bump_type").MinTimes(1).Return("patch")
	eng := engineBase{
		Config: fakeConfig,
	}
...
func TestEngineBase_BumpVersion_Minor(t *testing.T) {
	...
	mockCtrl := gomock.NewController(t)
	fakeConfig := mock_config.NewMockInterface(mockCtrl)
	fakeConfig.EXPECT().GetString("version_bump_type").MinTimes(1).Return("minor")
	eng := engineBase{
		Config: fakeConfig,
	}
...
func TestEngineBase_BumpVersion_Major(t *testing.T) {
	...
	mockCtrl := gomock.NewController(t)
	fakeConfig := mock_config.NewMockInterface(mockCtrl)
	fakeConfig.EXPECT().GetString("version_bump_type").MinTimes(1).Return("major")
	eng := engineBase{
		Config: fakeConfig,
	}
